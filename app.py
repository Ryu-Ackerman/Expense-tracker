...
import json
import os
from modifier import modify, track_limit, negative_checker, change_limit
...
class Checker:
    ...
    def currency_changer(self):

        new_c = input(f'Enter the new currency (current currency -> {self.currency()}): ')

        with open('category.json') as f:
            reader = json.load(f)

        reader['currency']['type'] = new_c

        with open('category.json', 'w') as f:
            json.dump(reader,f,indent=4)
            print(f'Successfully set currency to {new_c}')
...
month = f"0{date_s.month}" if date_s.month < 10 else str(date_s.month)
day = f"0{date_s.day}" if date_s.day < 10 else str(date_s.day)
...
def new():
    ...
    while True:
        ...
        with open("tracker.csv", 'a', newline="") as f:
            ...
            category, amount= get_data()#category = first, amount = second
        ...
        with open('category.json') as f:
            ...
            if category in fjson:
                ...
                namount = fjson[category]['amount'] + amount

            else:

                namount = amount

            fjson['total_money']['t'] -= float(amount)
            fjson[category] = {
                'amount': namount
                }
        ...
        with open('date.json') as f:
            ...
            if day in reader:
                famount = reader[day]['amount'] + amount
...
def change_total():
    ...
    with open("category.json") as f:
        ...
        dict_ = json.load(f)
        user = float(input('Enter the amount you currently have: '))
        dict_['total_money']['t'] = user
...
funcs = {
    ...
    'new': new,
    'currency -c': checker.currency_changer,
